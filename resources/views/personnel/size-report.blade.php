...
@section('title', 'Personnel Size Report')
@section('admin')
            ...
            <div class="row align-items-center">
                <div class="col-md-12">
                    <div class="page-header-title">
                        <h5 class="m-b-10">Personnel Size Report</h5>
                    </div>
                    <ul class="breadcrumb">
                        <li class="breadcrumb-item"><a href="index.html"><i class="feather icon-home"></i></a></li>
                        <li class="breadcrumb-item"><a href="#!">Personnel</a></li>
                        <li class="breadcrumb-item"><a href="#!">Size Report</a></li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h5>Personnel Apparel Sizes</h5>
                </div>
                <div class="card-body">
                    <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#sizeReportModal">
                        View Size Report
                    </button>
                </div>
            </div>
            ...
            $('#sizeReportModal').on('shown.bs.modal', function() {
                if (!$.fn.DataTable.isDataTable('#personnelSizeTable')) {
                    $('#personnelSizeTable').DataTable({
                        processing: true,
                        serverSide: true,
                        ajax: "{{ route('personnel.size.report.data') }}", // You need to create this route
                        columns: [
...
{{--
    ...
    1. You will need to create a new route in `routes/web.php` pointing to a controller method:
       ...
       Route::get('/personnel/size-report', [PersonnelController::class, 'showSizeReport'])->name('personnel.size.report');

    2. In your `PersonnelController`, create the `showSizeReport` method to return this view.

    3. In `PersonnelController`, create the `getSizeReportData` method to handle the AJAX request from DataTables.
